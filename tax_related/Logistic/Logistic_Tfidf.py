...
#import psycopg2
#load_dotenv('/cluster/work/lawecon/Work/goessmann/pg_server/pg.env')
#
#
### Load dataset from database
#
#con = psycopg2.connect(database='statelaws')
#print("connect to statelaws")
#cur = con.cursor()
#sql = """select dc_identifier,tax_code,phrases from tax_code_phrased_2020"""
#cur.execute(sql)
...
def create_features(data,k,prediction_path,phrase_dict, standardize):  ## Select top 10000 features first and then apply chi2 selector
  ...
  tfidf = pickle.load(open('/cluster/work/lawecon/Work/lhan/tax_related/tfidf_both_50000.pickle','rb'))
...
## Select features using Chi sqaured
  ...
  pickle.dump(chi2_selector, open(prediction_path+"/chi2_selector_tfidf"+str(k)+append+".pickle", "wb"))
...
def model_train_save(X,y, features_selected,prediction_path,phrase_dict,k,upsampling=False,SMOTE_Sampling=False,calibrated=True, standardize = True, train_on_all = False):
  ...
  if train_on_all == False:
    ...
    if calibrated == False:
      ...
      clf = GridSearchCV(logistic, hyperparameters, cv=3, verbose=2, n_jobs=-1, pre_dispatch = 32)
    ...
    if calibrated == True:
      ...
      calibrated = CalibratedClassifierCV(model, method='sigmoid', cv=3, n_jobs = -1 )
    ...
    rf_y, rf_x = calibration_curve(y_test, y_prob[:,1], n_bins=20)
    calibration_plot(rf_x,rf_y, y_prob[:,1], 'Logistic Regression',prediction_path,calibrated=True)
    ## Print metrics
...
def main():
  ...
  path = "/cluster/work/lawecon/Work/lhan/tax_related"
  prediction_path = "/cluster/work/lawecon/Work/lhan/tax_related/Logistic/Prediction"
  os.chdir(path)
  data = pickle.load(open('df_lexis.pickle',"rb")) # dataframe
...
## Remove phsrases still in stop_words list
  
  # for i in range(0,len(data)):
    # for j in data[i][2]:
        # if phrase_dict[j] in list(stop_words)+us_states:
          # data[i][2].remove(j)
